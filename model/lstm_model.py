...
class LSTM_model(nn.Module):
    def __init__(self, input_size, hidden_size, hidden_layers, lstm_layers, fc_layers, num_classes, t_step=54, num_coordi=45):
        ...
        self.fc_output_1 = nn.Linear(t_step * num_coordi, 1000)
    ...
    def forward(self, x):
        ...
        out = out.reshape(B,-1)
        out = self.fc_output_1(out)
        out = self.fc_output_2(out)
        final = self.fc_output_3(out)
        return final

if __name__ == "__main__":
    lstm_model = LSTM_model(45,45,6,6, 0,5,54,45)
    input = torch.randn(2,54,45)  # --> batch-size, T, hidden_size 
